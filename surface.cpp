...
namespace Tmpl8 {
	...
	{
		memset(m_Buffer, a_Color, m_Width * m_Height * sizeof(m_Buffer[0]));
	}
	...
	void Surface::PrintCentre(const char* a_String, int y1, int size, Pixel color)
	{
		int x = (m_Width - (int)strlen(a_String) * 6 * size) / 2;
		Print(a_String, x, y1, size, color);
	}
	...
	void Surface::Print(const char* a_String, int x1, int y1, int scale, Pixel color)
	{
		...
		for (int i = 0; i < (int)(strlen(a_String)); i++, t += 6 * scale)
		{
			...
			for (int v = 0; v < 5; v++, c++, a += scale * m_Pitch)
				for (int h = 0; h < 5; h++)
					if (*c++ == 'o')
						//memset((a + h *  + m_Pitch), color, scale * sizeof(Pixel));
...
#define OUTCODE(x,y) (((x)<xmin)?1:(((x)>xmax)?2:0))+(((y)<ymin)?4:(((y)>ymax)?8:0))
	...
	void Surface::Line(const vec2& a, const vec2& b, Pixel color)
	{
		Line(a.x, a.y, b.x, b.y, color);
	}

	void Surface::Arrow(float x1, float y1, float x2, float y2, Pixel color)
	{
		Line(x1, y1, x2, y2, color);
		Box(static_cast<int>(x2 - 4), static_cast<int>(y2 - 4), static_cast<int>(x2 + 4), static_cast<int>(y2 + 4), color);
	}
	...
	{
		Plot(static_cast<int>(x), static_cast<int>(y), c);
	}
	...
	void Surface::Box(const vec2& a, const vec2& b, Pixel color)
	{
		Box(static_cast<int>(a.x), static_cast<int>(a.y), static_cast<int>(b.x), static_cast<int>(b.y), color);
	}

	void Surface::circle(int x1, int y1, int r, int w, Pixel color)
	{
		for (float i = 0; i < 10.0f * (float)r; ++i)
		{
			for (float t = 0; t < (float)w; ++t)
			{
				const int x = static_cast<int>(cos(i) * (r + t * 0.5f));
				const int y = static_cast<int>(sin(i) * (r + t * 0.5f));
				Plot(x + x1, y + y1, color);
			}
		}
	}

	void Surface::circle(int x1, int y1, int rx, int ry, int w, Pixel color)
	{
		for (float i = 0; i < 10.0f * (float)max(rx, ry); ++i)
		{
			for (float t = 0; t < (float)w; ++t)
			{
				const int x = static_cast<int>(cos(i) * (rx + t * 0.5f));
				const int y = static_cast<int>(sin(i) * (ry + t * 0.5f));
				Plot(x + x1, y + y1, color);
			}
		}
	}
	...
	void Surface::Bar(int x1, int y1, int x2, int y2, Pixel c)
	{
		...
		const int amount = x2 - x1;
		for (int y = y1; y <= y2; y++)
		{
			for (int x = 0; x <= amount; x++) a[x] = c;
			a += m_Pitch;
		}
	}

	void Surface::Bar(const vec2& a, const vec2& b, Pixel color)
	{
		Bar(static_cast<int>(a.x), static_cast<int>(a.y), static_cast<int>(b.x), static_cast<int>(b.y), color);
	}
	...
	void Sprite::DrawScaled(const vec2& pos, const vec2& size, Surface* a_Target) const
	{
		DrawScaled(static_cast<int>(pos.x), static_cast<int>(pos.y),
			static_cast<int>(size.x), static_cast<int>(size.y), a_Target);
	}
	void Sprite::DrawScaled(const vec2& pos, const vec2& size, Pixel blend, Surface* a_Target) const
	{
		DrawScaled(static_cast<int>(pos.x), static_cast<int>(pos.y),
			static_cast<int>(size.x), static_cast<int>(size.y), blend, a_Target);
	}

	void Sprite::DrawScaled(int a_X, int a_Y, int a_Width, int a_Height, Surface* a_Target) const
	{
		const int height = a_Target->GetHeight();

		const Pixel* src = GetBuffer() + m_CurrentFrame * m_Width;
		...
		if (m_Flags & FLARE)
		{
			for (int x = 0; x < a_Width; x++) {
				...
				for (int y = 0; y < a_Height; y++)
				{
					...
					if (a_Target->GetWidth() * height < fx + (fy * a_Target->GetPitch()))
						continue;
					if (color & 0xffffff)
					{
						const Pixel old = a_Target->GetBuffer()[fx + (fy * a_Target->GetPitch())];

						a_Target->GetBuffer()[fx + (fy * a_Target->GetPitch())] = AddBlend(color, old);
					}
				}
			}
		}
		else
		{
			for (int x = 0; x < a_Width; x++) {
				...
				for (int y = 0; y < a_Height; y++)
				{
					...
					if (color & 0xffffff) a_Target->GetBuffer()
						[fx + (fy * a_Target->GetPitch())] = color;
				}
			}
		}
	}

	void Sprite::DrawScaled(int a_X, int a_Y, int a_Width, int a_Height, Pixel blendC, Surface* a_Target) const
	{
		const int height = a_Target->GetHeight();
		...
		for (int x = 0; x < a_Width; x++) {
			...
			for (int y = 0; y < a_Height; y++)
			{
				...
				if (color & 0xffffff) a_Target->GetBuffer()
					[fx + (fy * a_Target->GetPitch())] = blendC;
			}
		}
	}
	...
	SharedSprite::SharedSprite(Surface* a_Surface, unsigned a_NumFrames)
		: Sprite(a_Surface, a_NumFrames)
	{
		refCount = new int(1);
	}
	...
	SharedSprite::SharedSprite(const SharedSprite& Other)
		:Sprite(Other.m_Surface, Other.m_NumFrames)
	{
