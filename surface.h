...
#define BLUEMASK (0x0000ff)
	...
	public:
		...
		void PrintCentre(const char* a_String, int y1, int size, Pixel color);
		void Print(const char* a_String, int x1, int y1, Pixel color);
		void Print(const char* a_String, int x1, int y1, int scale, Pixel color);
		void Clear(Pixel a_Color);
		void Line(float x1, float y1, float x2, float y2, Pixel color);
		void Line(const vec2& a, const vec2& b, Pixel color);

		void Arrow(float x1, float y1, float x2, float y2, Pixel color);
		...
		void Box(const vec2& a, const vec2& b, Pixel color);
		void circle(int x1, int y1, int r, int w, Pixel color);
		void circle(int x1, int y1, int rx, int ry, int w, Pixel color);
		void Bar(int x1, int y1, int x2, int y2, Pixel color);
		void Bar(const vec2& a, const vec2& b, Pixel color);
	...
	public:
		...
		void DrawScaled(const vec2& pos, const vec2& size, Surface* a_Target) const;
		void DrawScaled(int a_X, int a_Y, int a_Width, int a_Height, Surface* a_Target)const;
		void DrawScaled(const vec2& pos, const vec2& size, Pixel blend, Surface* a_Target) const;
		void DrawScaled(int a_X, int a_Y, int a_Width, int a_Height, Pixel blend, Surface* a_Target)const;
		void SetFlags(unsigned int a_Flags) { m_Flags = a_Flags; }
		void SetFrame(unsigned int a_Index) { m_CurrentFrame = a_Index; }
		const unsigned int GetFrame() const { return m_CurrentFrame; }
	...
	class SharedSprite : public Sprite
	{
	public:
		SharedSprite(Surface* a_Surface, unsigned a_NumFrames);
		~SharedSprite() override;

		SharedSprite(const SharedSprite& Other);
	private:
