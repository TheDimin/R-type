...
#define TEMPLATE_VERSION	"D-Type"
...
#define unlikely(expr) __builtin_expect((expr),false)
	...
	public:
		...
		vec2Int(float x, float y) :x(static_cast<int>(x)), y(static_cast<int>(y)) {}
	};
	...
	public:
		...
		vec2 lerp(const vec2& otherVec, float time) const
		{
			return (*this) + (otherVec - (*this)) * time;
		}
		static vec2 lerp(const vec2& a, const vec2& b, float time) { return a * b + b * (1.f - time); }
	};
